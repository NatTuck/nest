defmodule Nest.PersistenceAgentsTest do
  ...
  setup do
    # Force a space_id to be created so the `:not_found` tests
    # that call `fetch_agent/2` etc. without ever calling
    # `agent_attrs/1` still have a valid integer to pass.
    _ = test_space_id()
    :ok
  end
  ...
  describe "insert_agent/1" do
    ...
    test "validates required fields" do
      assert {:error, %Ecto.Changeset{} = changeset} =
               Persistence.insert_agent(%{name: "", model: %{}, space_id: 0})
    ...
    test "returns :missing_required_field when :model is absent" do
      assert {:error, :missing_required_field} =
               Persistence.insert_agent(%{name: "no-model-agent", space_id: 0})

      assert {:error, :missing_required_field} =
               Persistence.insert_agent(%{space_id: 0})
    end
  end
  ...
  describe "fetch_agent_by_name/1" do
    ...
    test "returns :not_found for an unknown name" do
      assert {:error, :not_found} =
               Persistence.fetch_agent(Process.get(:nest_persistence_test_space_id), "never-seen")
    end
  end
  ...
  describe "list_agent_names/0" do
    test "returns one entry per persisted agent" do
      ...
      names = Persistence.list_agent_names_for_space(a.space_id)
  ...
  describe "build_attrs_for_start/1" do
    test "returns attrs suitable for Agent.start_link including last_compaction_index" do
      ...
      assert {:ok, _} = Persistence.record_compaction(test_space_id(), attrs.name, 2, 2)
    ...
    test "returns :not_found when no row exists" do
      assert {:error, :not_found} =
               Persistence.build_attrs_for_start(
                 Process.get(:nest_persistence_test_space_id),
  ...
  describe "update_agent_model/2" do
    test "replaces the jsonb model column with the new map" do
      ...
      assert :ok = Persistence.update_agent_model(test_space_id(), attrs.name, new_model)
    ...
    test "returns :not_found when the agent row is absent" do
      ...
      assert {:error, :not_found} =
               Persistence.update_agent_model(
                 Process.get(:nest_persistence_test_space_id),
                 "missing-agent",
                 new_model
               )
    ...
    test "touches updated_at on the existing row" do
      ...
      :ok = Persistence.update_agent_model(test_space_id(), attrs.name, new_model)
  ...
  describe "fetch_all_agents/0" do
    test "lists every persisted agent, ordered by name" do
      ...
      all = Persistence.fetch_all_agents_for_space(attrs_a.space_id) |> Enum.map(& &1.name)
