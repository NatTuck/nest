defmodule Nest.Persistence do
  ...
  @doc """
  Delete the agent row for `name`. Returns `:ok` whether or not
  the row existed.

  `Nest.Agents.delete_agent/1` only terminates the BEAM
  process. Callers that want to also remove the DB row
  (e.g., tests verifying the post-delete `get_info/1` path,
  or the lobby's "remove agent" UX) call this after the
  process exits.
  """
  @spec delete_agent_by_name(String.t()) :: :ok
  def delete_agent_by_name(name) do
    {_count, _} = Repo.delete_all(from(a in PersistedAgent, where: a.name == ^name))
    :ok
  end
