defmodule NestWeb.LobbyChannel.Authz do
  ...
  Pure functions over `(name, current_user)` — no socket,
  no logging. Kept in a sibling file so `LobbyChannel`
  ...
  def authorize_owner(name, current_user) do
    case fetch_visibility(name) do
  ...
  def authorize_owner_or_shared(name, current_user) do
    case fetch_visibility(name) do
  ...
  # raises `:exit` in that case — see `fetch_visibility/1`).
  defp fetch_from_db(name) do
    case Nest.Persistence.fetch_agent_by_name(name) do
