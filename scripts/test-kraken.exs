#!/usr/bin/env elixir
# Test script for kraken provider with dice rolling tool
# Usage: mix run scripts/test-kraken.exs
...
defmodule KrakenScript do
  ...
  def run do
    ...
    provider =
      case DotConfig.get_provider(config, "kraken") do
        nil ->
          IO.puts("Error: kraken provider not found in config")
          System.halt(1)
    ...
    client_config =
      try do
        {:ok, cfg} = ChatModel.from_provider("kraken", nil)
        cfg
      rescue
        e ->
          IO.puts("Error creating client config: #{Exception.message(e)}")
          System.halt(1)
      end
  ...
  defp do_run_with_tool_loop(%RunRequest{} = request, client_config, opts, tools, budget) do
